...
{
    public async Task<IReadOnlyList<DepartmentDto>> DepartmentsAsync(CancellationToken ct)
    {
        return mapper.Map<IReadOnlyList<DepartmentDto>>(await departments.ListAsync(x => !x.IsDeleted, ct));
    }

    public async Task<DepartmentDto> CreateDepartmentAsync(DepartmentRequest r, CancellationToken ct)
    {
        if (await departments.ExistsAsync(x => x.Code == r.Code && !x.IsDeleted, ct))
        {
        ...
        await departments.InsertAsync(e, ct);
        return mapper.Map<DepartmentDto>(e);
    }
    public async Task<DepartmentDto> UpdateDepartmentAsync(string id, DepartmentRequest r, CancellationToken ct)
    {
        ...
        if (r.ParentId == id)
        {
            throw new BusinessException("部门不能以自身为上级");
        }

        Apply(e, r);
        EntityAudit.Update(e, clock, user);
        await departments.UpdateAsync(e, ct);
        return mapper.Map<DepartmentDto>(e);
    }
    public async Task DeleteDepartmentAsync(string id, CancellationToken ct)
    {
        var e = await departments.GetByIdAsync(IdParser.Parse(id), ct) ?? throw new NotFoundException("部门不存在");
        var hasChildDepartment = await departments.ExistsAsync(
            department => department.ParentId == e.Id && !department.IsDeleted,
            ct);
        var hasEmployee = await employees.ExistsAsync(
            employee => employee.DepartmentId == e.Id && !employee.IsDeleted,
            ct);

        if (hasChildDepartment || hasEmployee)
        {
            throw new ConflictException("部门已被引用，只能停用");
        }

        e.IsDeleted = true;
        e.DeletedAt = clock.UtcNow;
        e.DeletedBy = user.UserId;
        await departments.UpdateAsync(e, ct);
    }
    ...
    public async Task<PositionDto> CreatePositionAsync(PositionRequest r, CancellationToken ct)
    {
        ...
        if (!await departments.ExistsAsync(x => x.Id == did && !x.IsDeleted, ct))
        {
            throw new NotFoundException("部门不存在");
        }

        if (await positions.ExistsAsync(x => x.Code == r.Code && !x.IsDeleted, ct))
        {
        ...
        EntityAudit.Create(e, ids, clock, user);
        await positions.InsertAsync(e, ct);
        return mapper.Map<PositionDto>(e);
    }
    public async Task<PositionDto> UpdatePositionAsync(string id, PositionRequest r, CancellationToken ct)
    {
        ...
        e.Remark = r.Remark;
        EntityAudit.Update(e, clock, user);
        await positions.UpdateAsync(e, ct);
        return mapper.Map<PositionDto>(e);
    }
    public async Task DeletePositionAsync(string id, CancellationToken ct)
    {
        var e = await positions.GetByIdAsync(IdParser.Parse(id), ct) ?? throw new NotFoundException("岗位不存在");
        if (await employees.ExistsAsync(x => x.PositionId == e.Id && !x.IsDeleted, ct))
        {
            throw new ConflictException("岗位已被引用，只能停用");
        }
    ...
    private static void Apply(Department e, DepartmentRequest r)
    {
        ...
        e.Name = r.Name.Trim();
        e.Code = r.Code.Trim();
        e.LeaderEmployeeId = IdParser.ParseNullable(r.LeaderEmployeeId, "leaderEmployeeId");
