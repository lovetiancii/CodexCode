...
public sealed class SqlSugarRepository<T>(ISqlSugarClient db) : IRepository<T>
    where T : class, new()
{
    ...
    public Task<bool> ExistsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return db.Queryable<T>().Where(predicate).AnyAsync();
    }
    ...
    public async Task<(IReadOnlyList<T> Items, long Total)> PageAsync(
        Expression<Func<T, bool>> predicate,
        int pageNumber,
        int pageSize,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (array.Length > 0)
        {
            _ = await db.Insertable(array).ExecuteCommandAsync();
        }
    }
    ...
    public Task<int> UpdateWhereAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return db.Updateable(entity).Where(predicate).ExecuteCommandAsync();
    }
    ...
    public Task<int> DeleteWhereAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        return db.Deleteable<T>().Where(predicate).ExecuteCommandAsync();
    }
}
