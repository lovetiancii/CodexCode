...
public sealed class SnowflakeIdGenerator : ISnowflakeIdGenerator
{
    ...
    {
        if (nodeId is < 0 or > 1023) throw new ArgumentOutOfRangeException(nameof(nodeId));
        _node = nodeId;
    }
    public long NextId()
    {
        lock (_sync)
        {
            ...
            if (now < _last) throw new InvalidOperationException("系统时钟回拨，雪花 ID 生成被拒绝");
            if (now == _last) { _sequence = (_sequence + 1) & 4095; if (_sequence == 0) do { now = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(); } while (now <= _last); }
            else _sequence = 0;
            _last = now;
