    ...
    IOptions<FileStorageOptions> options) : IFileStorage
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var directory = Path.Combine(_root, date);
        Directory.CreateDirectory(directory);
        var key = $"{date}/{Guid.NewGuid():N}{extension}".Replace('\\', '/');
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(path))
        {
            throw new FileNotFoundException("存储文件不存在");
        }
    ...
    private string Resolve(string key)
    {
        ...
        {
            throw new InvalidOperationException("无效存储键");
        }
