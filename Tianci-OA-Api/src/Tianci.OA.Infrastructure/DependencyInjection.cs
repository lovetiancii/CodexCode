...
public static class DependencyInjection
{
        ...
        IConfiguration configuration)
    {
        ...
        {
            throw new InvalidOperationException("Jwt:Secret 必须至少 32 字节，并通过环境变量覆盖");
        }

        services.Configure<FileStorageOptions>(configuration.GetSection("FileStorage"));
        var keyPath = Path.GetFullPath(
            configuration["DataProtection:KeyPath"] ?? "App_Data/keys");

        Directory.CreateDirectory(keyPath);
        services
            .AddDataProtection()
            .PersistKeysToFileSystem(new DirectoryInfo(keyPath))
            .SetApplicationName("Tianci.OA");

        services.AddHttpContextAccessor();
        services.AddAutoMapper(_ => { }, typeof(MappingProfile));
        services.AddSingleton<IClock, SystemClock>();
        services.AddSingleton<ISnowflakeIdGenerator>(_ =>
            new SnowflakeIdGenerator(
                configuration.GetValue("Snowflake:NodeId", 1)));
        services.AddScoped<ICurrentUser, CurrentUser>();
        services.AddScoped<ISensitiveDataProtector, SensitiveDataProtector>();
        services.AddSingleton<Tianci.OA.Application.Abstractions.ICacheService>(_ =>
                ...
                options.Events = new JwtBearerEvents
                {
                    OnTokenValidated = async context =>
                    {
                        ...
                        var repository = context.HttpContext.RequestServices
                            .GetRequiredService<IRepository<SysUser>>();
                        var user = await repository.FirstAsync(
                            user => user.Id == id && !user.IsDeleted);

                        if (user is null
                            || user.Status != Tianci.OA.Domain.Common.UserStatus.Enabled
    ...
    private static string TableName(Type type)
    {
        ...
        {
            nameof(SysUser) => "sys_user",
            nameof(SysRole) => "sys_role",
            nameof(SysMenu) => "sys_menu",
            nameof(SysUserRole) => "sys_user_role",
            nameof(SysRoleMenu) => "sys_role_menu",
            nameof(Department) => "department",
            nameof(Position) => "position",
            nameof(Employee) => "employee",
            nameof(EmployeeEntry) => "employee_entry",
            nameof(Resume) => "resume",
            nameof(InterviewRecord) => "interview_record",
            nameof(EmployeeContract) => "employee_contract",
            nameof(SysFile) => "sys_file",
            nameof(OperationLog) => "operation_log",
            nameof(WorkflowInstance) => "workflow_instance",
            nameof(WorkflowNode) => "workflow_node",
            nameof(WorkflowRecord) => "workflow_record",
