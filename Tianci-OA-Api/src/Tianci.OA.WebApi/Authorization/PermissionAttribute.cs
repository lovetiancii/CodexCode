...
{
    public PermissionAttribute(string permission) : base(typeof(PermissionFilter)) => Arguments = [permission];
}
...
public sealed class PermissionFilter(IPermissionService permissions, string permission) : IAsyncAuthorizationFilter
{
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        ...
        if (!long.TryParse(claim, out var userId) || !await permissions.HasPermissionAsync(userId, permission)) context.Result = new ForbidResult();
    }
}
