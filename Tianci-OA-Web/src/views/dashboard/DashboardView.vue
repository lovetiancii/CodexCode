...
const metrics = computed(() => [
  { label: '员工总数', value: state.employeeTotal, note: '当前人员档案', color: '#2563eb', bg: '#eaf2ff', path: '/employees', permission: 'employee:view' },
  { label: '简历总数', value: state.resumes, note: '招聘人才库', color: '#7c3aed', bg: '#f1ebff', path: '/recruitment/resumes', permission: 'resume:view' },
  { label: '待筛选', value: state.screening, note: '需要及时处理', color: '#e38a13', bg: '#fff5df', path: '/recruitment/resumes?status=1', permission: 'resume:view' },
  { label: '面试中', value: state.interviewing, note: '含待安排场次', color: '#16a269', bg: '#e7f8f0', path: '/recruitment/interviews', permission: 'resume:view' },
  { label: '合同即将到期', value: state.expiring, note: '未来 30 天', color: '#e34a4a', bg: '#ffeded', path: '/contracts?expiring=true', permission: 'contract:view' },
...
const todos = computed(() => [
  { title: `${state.screening} 份简历等待筛选`, category: '招聘', path: '/recruitment/resumes?status=1', danger: state.screening > 0, permission: 'resume:view' },
  { title: `${state.interviewing} 位候选人处于面试流程`, category: '面试', path: '/recruitment/interviews', danger: false, permission: 'resume:view' },
  { title: `${state.expiring} 份合同将在 30 天内到期`, category: '合同', path: '/contracts?expiring=true', danger: state.expiring > 0, permission: 'contract:view' },
].filter((item) => auth.has(item.permission)))
...
async function load() {
  ...
  const emptyPage = { items: [], pageNumber: 1, pageSize: 1, total: 0 }
  const calls = await Promise.allSettled([
    auth.has('employee:view') ? employeeApi.list({ pageNumber: 1, pageSize: 100 }) : Promise.resolve(emptyPage),
    auth.has('resume:view') ? resumeApi.list({ pageNumber: 1, pageSize: 1 }) : Promise.resolve(emptyPage),
    auth.has('resume:view') ? resumeApi.list({ status: RESUME_STATUS.Submitted, pageNumber: 1, pageSize: 1 }) : Promise.resolve(emptyPage),
    auth.has('resume:view') ? resumeApi.list({ status: RESUME_STATUS.InterviewPending, pageNumber: 1, pageSize: 1 }) : Promise.resolve(emptyPage),
    auth.has('resume:view') ? resumeApi.list({ status: RESUME_STATUS.Interviewing, pageNumber: 1, pageSize: 1 }) : Promise.resolve(emptyPage),
    auth.has('resume:view') ? resumeApi.list({ status: RESUME_STATUS.Hired, pageNumber: 1, pageSize: 1 }) : Promise.resolve(emptyPage),
    auth.has('contract:view') ? contractApi.expiring(30) : Promise.resolve([]),
  ])
...
<template>
  <div class="page dashboard-page" v-loading="loading">
    ...
    <div class="dashboard-grid">
      <section v-if="auth.has('resume:view')" class="panel funnel-panel">
      ...
      <section v-if="auth.has('employee:view')" class="panel trend-panel">
      ...
      <section class="panel quick-panel">
        ...
        <div class="quick-grid">
          ...
          <button v-permission="'resume:schedule'" @click="router.push('/recruitment/interviews?action=schedule')"><el-icon><Calendar /></el-icon><span>安排面试</span></button>
