...
开发种子 `admin` 使用 ASP.NET Core `PasswordHasher<SysUser>` 生成的 PBKDF2 哈希，状态为启用且无需二次初始化；默认密码仅用于首次开发启动，必须在首次登录后立即修改。`INSERT IGNORE` 保证重复执行脚本不会覆盖已修改的管理员密码。共享、测试和生产环境不得继续使用文档默认密码。
...
初始化后应做以下验证：所有表字符集为 `utf8mb4`；不存在 `AUTO_INCREMENT`；外键完整；开发默认管理员可登录；修改密码后旧默认密码立即失效。
